<template>
  <div class="page-container">
      ...
      <template v-slot:content>
        <pre>
          13，Vue2.0和Vue3.0为了实现响应式的区别和采用的api的区别
            ...
            那看起来灵敏，不过，需要一些 Proxy 的知识才能理解！所以让我们深入了解一下。
            有很多关于 Proxy 的文档，但你真正需要知道的是，Proxy 是一个对象，它包装了另一个对象，
            并允许你拦截对该对象的任何交互。我们这样使用它：new Proxy(target, handler)
        </pre>
      </template>
    </t-l-collapse>

    <t-l-collapse title="5,移动端兼容问题">
      <template v-slot:content>
        <pre>
          10，苹果的适配区域
    ...
    <t-l-collapse title="6,开发插件的了解程度">
      <template v-slot:content>
        <pre>
          4, SSR和CSR 概念
